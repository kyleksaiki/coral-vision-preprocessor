 ...
 * "card" are ever seeded, so no silt label can appear off-card, and growth (which only spreads
 * through on-card weak candidates) cannot leak across a card edge.</p>
 */
public class LabelSilt {
    ...
    /**
     * Labels silt-like pixels across the whole image (no card mask). Kept for backward
     * compatibility; prefer {@link #labelSiltPixels(Mat, char[][], Mat)}.
     *
    ...
    public static char[][] labelSiltPixels(Mat bgrImage, char[][] labels) {
        return labelSiltPixels(bgrImage, labels, null);
    }

    /**
     * Labels silt-like pixels in the provided label grid, restricted to the card mask.
     *
     ...
     * {@code 'C'} or {@code 'c'}, and never seeds a pixel where the card mask is 0.</p>
     *
     ...
     *                 Pixels where this is 0 are never labeled silt.
     * @return the same {@code labels} array after silt pixels have been labeled
     */
    public static char[][] labelSiltPixels(Mat bgrImage, char[][] labels, Mat cardMask) {
    ...
    private static byte[] cardBuffer(Mat cardMask, int rows, int cols) {
        ...
        if (cardMask.rows() != rows || cardMask.cols() != cols) {
            System.err.println("[LabelSilt] card mask size != image size; ignoring mask.");
            return null;
        }
