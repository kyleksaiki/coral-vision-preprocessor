 ...
 * "card" are ever seeded as candidates, so no algae label can appear off-card. Because the
 * growth pass only spreads through weak candidates (which are all on-card), it cannot leak
 * across a card edge either.</p>
 */
public class LabelAlgae {
    ...
    /**
     * Labels algae-like pixels across the whole image (no card mask). Kept for backward
     * compatibility; prefer {@link #labelAlgaePixels(Mat, char[][], Mat)}.
     *
    ...
    public static char[][] labelAlgaePixels(Mat bgrImage, char[][] labels) {
        return labelAlgaePixels(bgrImage, labels, null);
    }

    /**
     * Labels algae-like pixels in the provided label map, restricted to the card mask.
     *
     ...
     * {@code 'c'}, and never seeds a pixel where the card mask is 0.</p>
     *
     ...
     *                 Pixels where this is 0 are never labeled algae.
     * @return the same {@code labels} array after algae pixels have been labeled
     */
    public static char[][] labelAlgaePixels(Mat bgrImage, char[][] labels, Mat cardMask) {
        ...
        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < cols; c++) {
                ...
                if (cardBuf != null && cardBuf[r * cols + c] == 0) {
                    continue;
                }
    ...
    private static byte[] cardBuffer(Mat cardMask, int rows, int cols) {
        ...
        if (cardMask.rows() != rows || cardMask.cols() != cols) {
            System.err.println("[LabelAlgae] card mask size != image size; ignoring mask.");
            return null;
        }
