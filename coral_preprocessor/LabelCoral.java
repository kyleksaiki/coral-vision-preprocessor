 ...
 * </ul>
 *
 * <p><b>Card-mask gating.</b> When a card mask is supplied, only pixels the model called
 * "card" are even considered. This matters here in particular: the "catch-all dark" rule
 * below deliberately accepts very dark pixels as coral, which on a whole image also grabs the
 * dark mesh net. Restricted to the card footprint, that mesh is off-card and is left alone.</p>
 */
public class LabelCoral {
    ...
    // Catch-all dark coral.
    // Underwater dark/black coral is often blue or green cast, so it fails the warm-brown
    // rules above. Any pixel this dark is treated as coral. Raise these to catch more
    // (on a whole image this also labels the dark net as coral; the card mask keeps it on-card).
    private static final int DARK_CORAL_L_MAX = 110;
    private static final int DARK_CORAL_V_MAX = 130;
    ...
    /**
     * Labels coral pixels across the whole image (no card mask). Kept for backward
     * compatibility; prefer {@link #labelCoralPixels(Mat, char[][], Mat)} so distractors
     * outside the cards can never be labeled.
     *
    ...
    public static char[][] labelCoralPixels(Mat bgrImage, char[][] labels) {
        return labelCoralPixels(bgrImage, labels, null);
    }

    /**
     * Labels coral pixels in the given label grid, restricted to the card mask.
     *
     ...
     * </ul>
     *
     * <p>Pixels that pass coral color checks are labeled as {@code 'C'}, but ONLY where the
     * card mask is non-zero. After initial labeling, small connected coral components are
     * removed.</p>
     *
     * @param bgrImage OpenCV image in BGR color format (the normalized / CLAHE image)
     * @param labels   existing label grid to update; dimensions should match the image
     * @param cardMask CV_8UC1 mask (255 = card); pass {@code null} to label the whole image.
     *                 Pixels where this is 0 are never labeled coral.
     * @return the same {@code labels} array after coral pixels have been labeled
     */
    public static char[][] labelCoralPixels(Mat bgrImage, char[][] labels, Mat cardMask) {
        ...
        // ===== CARD-MASK GATE: pull the mask into a flat array (null = whole image). =====
        byte[] cardBuf = cardBuffer(cardMask, rows, cols);
        ...
        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < cols; c++) {
                ...
                // ===== CARD-MASK GATE: skip any pixel the model did not call "card". =====
                boolean onCard = (cardBuf == null) || (cardBuf[r * cols + c] != 0);
    ...
    /**
     * Pulls a CV_8UC1 card mask into a flat byte array aligned to the image (row-major).
     * Returns {@code null} (meaning "label the whole image") when the mask is absent, empty,
     * or a different size than the image.
     */
    private static byte[] cardBuffer(Mat cardMask, int rows, int cols) {
        if (cardMask == null || cardMask.empty()) {
            return null;
        }
        if (cardMask.rows() != rows || cardMask.cols() != cols) {
            System.err.println("[LabelCoral] card mask size != image size; ignoring mask.");
            return null;
        }
        byte[] buf = new byte[rows * cols];
        cardMask.get(0, 0, buf);
        return buf;
    }
    ...
    private static boolean isCoralBrown(
        ...
        boolean darkCoral = l <= DARK_CORAL_L_MAX && v <= DARK_CORAL_V_MAX;
