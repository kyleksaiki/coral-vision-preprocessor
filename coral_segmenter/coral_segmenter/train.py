"""Train the coral-segmentation model on CLAHE card crops.
...
from dataset import CoralCropDataset, eval_tf, make_splits, train_tf
...
def run(cfg):
    ...
    print("Split by tray:", info)
    print(f"Crops -> train {len(train_items)}, val {len(val_items)}, test {len(test_items)}")
    if len(train_items) == 0 or len(val_items) == 0:
        raise RuntimeError("Need at least one tray in train and one in val. Add more data "
                           "or set val_dives/test_dives in config.yaml.")

    h, w = cfg["train"]["img_height"], cfg["train"]["img_width"]
    nw = cfg["train"]["num_workers"]
    train_loader = DataLoader(CoralCropDataset(train_items, train_tf(h, w)),
                              batch_size=cfg["train"]["batch_size"], shuffle=True,
                              num_workers=nw, drop_last=False)
    val_loader = DataLoader(CoralCropDataset(val_items, eval_tf(h, w)),
    ...
    # BCE keeps every pixel roughly right; Dice cares about overlap of the coral shape,
    # which matters more here because coral is a smaller fraction of a crop than cards were.
